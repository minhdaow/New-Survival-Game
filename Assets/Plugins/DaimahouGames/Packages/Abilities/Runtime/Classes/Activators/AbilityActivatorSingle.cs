...
namespace DaimahouGames.Runtime.Abilities
{
    ...
    public class AbilityActivatorSingle : AbilityActivator
    {
        ...
        [SerializeField] protected Indicator m_RangeIndicator;
        ...
        public override async Task Activate(ExtendedArgs args)
        {
            ...
            if(!args.Has<Target>()) {
                var range = (float) ability.GetRange(args);
                var castRangeMarker = m_RangeIndicator != null
                    ? m_RangeIndicator.Get(args, ability.Caster.Position, new Vector3(range * 2, 1f, range * 2))
                    : null;
                
                await ability.Targeting.ProcessInput(args);
            
                if (castRangeMarker) {
                    castRangeMarker.SetActive(false);
                }
            }
            ...
            else if (ability.Targeting.ShouldCheckRange && ability.IsInRange(args) == false) {
                ability.Cancel();
            }
