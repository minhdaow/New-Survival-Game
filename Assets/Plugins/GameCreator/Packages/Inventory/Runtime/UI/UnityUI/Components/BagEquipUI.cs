...
namespace GameCreator.Runtime.Inventory.UnityUI
{
        ...
        ISubmitHandler
    {
        ...
        [SerializeField] public bool inputEnabled = true; // Added bool to enable/disable input
        ...
        private void Awake()
        {
            if (inputEnabled) // Check if input is enabled
            {
        ...
        {
            if (!inputEnabled) return; // Check if input is enabled
            ...
            switch (this.m_OnClick)
            {
                ...
                case EnumOnClick.Select: this.Select(); break; // Handle Select case
        ...
        public void OnSubmit(BaseEventData data)
        {
            ...
            switch (this.m_OnSubmit)
            {
                ...
                case EnumOnSubmit.Select: this.Select(); break; // Handle Select case
                default: throw new ArgumentOutOfRangeException();
            }
        }
        ...
        public void Select()
        {
            ...
            IdString runtimeItemID = this.Bag.Equipment.GetSlotRootRuntimeItemID(this.m_EquipmentIndex.Index);
            if (string.IsNullOrEmpty(runtimeItemID.String)) return;

            RuntimeItem runtimeItem = this.Bag.Content.GetRuntimeItem(runtimeItemID);
            if (runtimeItem == null) return;

            BagCellUI cellUI = this.FindBagCellUI(runtimeItem);
            if (cellUI != null)
            {
                cellUI.Select(); // Invoke Select on the BagCellUI to trigger SelectedCellUI
            }
        }

        private BagCellUI FindBagCellUI(RuntimeItem runtimeItem)
        {
            foreach (BagCellUI cellUI in FindObjectsOfType<BagCellUI>())
            {
                if (cellUI.Cell != null && cellUI.Cell.RootRuntimeItem == runtimeItem)
                {
                    return cellUI;
                }
            }
