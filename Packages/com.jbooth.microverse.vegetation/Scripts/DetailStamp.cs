...
#if UNITY_EDITOR
        /// <summary>
        /// Copy the noise settings from one child to all the siblings of it.
        /// eg if you have a detail stamp parent with several grass detail stamps you can transfer the settings from one grass detail stamp to all the others
        /// </summary>
        [ContextMenu("Transfer Noise to Siblings")]
        void TransferNoiseToSiblings()
        {
            ...
            for (int i = 0; i < parent.childCount; i++)
            {
                ...
                DetailStamp stamp = child.GetComponent<DetailStamp>();

                if (stamp == null)
                    continue;

                // clone and transfer data
                stamp.filterSet.weightNoise = this.filterSet.weightNoise.Clone() as Noise;
                stamp.filterSet.weight2Noise = this.filterSet.weight2Noise.Clone() as Noise;
                stamp.filterSet.weight3Noise = this.filterSet.weight3Noise.Clone() as Noise;
